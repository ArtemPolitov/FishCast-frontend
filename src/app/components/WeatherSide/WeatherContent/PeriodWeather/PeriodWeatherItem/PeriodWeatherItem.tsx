...
const PeriodWeatherItem: React.FC<PeriodWeatherItemProps> = ({ periodWeatherItemData }) => {
  ...
  return (
    <div className={s.periodWeatherItem}>
      <p className={s.time}>
        {getKyivHour(periodWeatherItemData.dt) < 10
          ? `0${getKyivHour(periodWeatherItemData.dt)}:00`
          : `${getKyivHour(periodWeatherItemData.dt)}:00`}
      </p>
      <div className={s.weatherIconContainer}>
        <Image
          src={`https://openweathermap.org/img/wn/${periodWeatherItemData.weather[0].icon}@2x.png`}
          alt={'weather icon'}
          height={30}
          width={30}
